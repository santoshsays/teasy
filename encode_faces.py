# code to encode the registered images in dataset to the encodings.pickle file 
...
args = vars(ap.parse_args('-i dataset -e encodings.pickle -d hog'.split()))
#code to perform only not modeified user encodings
imagePaths = list(paths.list_images(args["dataset"]))
#code to write all the file paths to file 
for item in imagePaths:    
    with open("alldownload.txt","a") as f:
        f.write(item +"\n")
    f.close()
f1=open("alldownload.txt","r+")
f2=open("check.txt")
...
if not enc_list:
    ...
    original = '/home/pi/tapp/dataset/'
    dest= '/home/pi/tapp/d_dataset/'
